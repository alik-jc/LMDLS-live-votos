        ...
        <div className="max-w-7xl mx-auto px-4 mt-16">
            <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                    <AlertOctagon className="text-[#8c3034]" size={28} />
                    <h2 className={`text-2xl font-serif ${isDark ? 'text-white' : 'text-gray-900'}`}>Participantes eliminados hasta el momento</h2>
                </div>
                <span className="text-[#8c3034] text-sm bg-[#8c3034]/20 px-3 py-1 rounded-full border border-[#8c3034]/30">
                ...
                {candidates.map((p) => (
                    <div key={p.name} className={`border rounded-lg overflow-hidden relative group transition-all duration-300 ${isDark ? 'bg-[#111] border-[#8c3034]/20 hover:border-[#8c3034]/50' : 'bg-white border-red-200 hover:border-[#8c3034]/50'}`}>
                        ...
                        <div className="p-4 flex items-center gap-4 relative z-10">
                            ...
                            <div>
                                <h3 className={`font-bold text-lg leading-tight ${isDark ? 'text-white' : 'text-gray-900'}`}>{p.name}</h3>
